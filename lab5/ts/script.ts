...
class Product {
    ...
    Init(): any {
        ...
        a.setAttribute("stock", this.inStock.toString());
        a.setAttribute("href", "#buyModal");
        a.setAttribute("class", "btn btn-primary");
        a.setAttribute("data-toggle", "modal");
        a.setAttribute("onclick", "WantBuy(this.id, " + this.inStock.toString() + ")");
        a.innerHTML = "Купить";
...
enum Material { Silk = "Шелк", Polyester = "Синтетика", Coton = "Хлопок" };
...
//Группировка по наличии гарантии
function CheckGuarantee(flag: boolean) {
    document.getElementById('rowts').innerHTML = "";
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof PowerTool && (<PowerTool>productList[i]).params.guarantee.length > 0) (<PowerTool>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
            productList[i].Init();
...
interface BedclothesParams {
    size: number; //размер
    material: Material; //материал
}

class Bedclothes extends Product {
    params: BedclothesParams;
    constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public prms: BedclothesParams ) {
        super(id, name, price, description, inStock);
        this.params = prms
        this.Init();
    }
    ...
    Init() {
        ...
        if (!!this.params.material) {
            let p = document.createElement("p");
            p.setAttribute("class", "card-text text-info m-0");
            p.innerHTML = `Материал: ${this.params.material}`;
            obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
        }
        if (!!this.params.size) {
            let p = document.createElement("p");
            p.setAttribute("class", "card-text text-info m-0");
            p.innerHTML = `Разер: ${this.params.size}`;
            obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
        }
...
interface PowerToolParams {
    kit: string; //размер
    guarantee: string; //материал
}

class PowerTool extends Product {
    params: PowerToolParams;
    constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public prms: PowerToolParams) {
        super(id, name, price, description, inStock);
        this.params = prms
        this.Init();
    }
    Init() {
        ...
        if (!!this.params.kit) {
            let p = document.createElement("p");
            p.setAttribute("class", "card-text text-info m-0");
            p.innerHTML = `Комплект: ${this.params.kit}`;
            obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
        }
        if (!!this.params.guarantee) {
            let p = document.createElement("p");
            p.setAttribute("class", "card-text text-info m-0");
            p.innerHTML = `Гарантия: ${this.params.guarantee}`;
            obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
        }

        if (document.getElementById('hasGuarantee') == null && this.params.guarantee.length > 0) {
            let inp = document.createElement("input");
            inp.setAttribute("type", "checkbox");
            inp.setAttribute("id", "hasGuarantee");
            inp.setAttribute("onclick", "CheckGuarantee(this.checked)");

            let lab = document.createElement("p");
            lab.appendChild(inp);
            lab.innerHTML += "С гарантией<br>";
...
class Basket {
    ...
    Add(val: number): boolean {
        ...
        if (isNaN(num) || !((num ^ 0) === num) || num == 0 || productList[val].inStock < num) {
            ...
            this.list[itemIndex] = { id: val, quantity: num };
...
function WantBuy(val: any, count: any) {
...
let productList: Product[] = [
    ...
    new Bedclothes(9, "Постельное белье", 199, "постельное белье 1.5: наволочка, пододеяльник, простыня", 4,{size: 1, material: Material.Coton}),
    new PowerTool(10, "Болгарка", 999, "ушм 900W 125 димаметр",42, {guarantee: "12 месяцев", kit: "сумка, ключ, ушм, запасные щетки"}),
